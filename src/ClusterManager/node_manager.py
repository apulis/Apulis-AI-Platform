...
def get_job_gpu_usage(jobId):
    try:
        ...
        url = """http://"""+hostaddress+""":9091/prometheus/api/v1/query?query=avg(avg_over_time(task_device_percent{pod_name=""" + jobId + """"}[4h])) by (pod_name, instance, username)"""
        ...
        for node_name, node_status in nodes_status.iteritems():
            if node_status["unschedulable"]:
                ...
                gpuMapping[node_status["gpuType"]]["detail"].append({"nodeName": node_name,
                                                                     "capacity": ResourceInfo(node_status["gpu_capacity"]).ToSerializable(),
                                                                     "allocatable": 0})
            else:
                ...
                gpuMapping[node_status["gpuType"]]["detail"].append({"nodeName": node_name,
                                                                     "capacity": ResourceInfo(node_status["gpu_capacity"]).ToSerializable(),
                                                                     "allocatable": ResourceInfo(node_status["gpu_allocatable"]).ToSerializable() - ResourceInfo(node_status["gpu_used"]).ToSerializable()})
