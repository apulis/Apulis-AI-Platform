        ...
        if oneJob["type"] == "image" and oneJob["targetFormat"]=="coco":
            try:
                ...
                if judge_datasets_is_private(oneJob["projectId"],oneJob["datasetId"]):
                    ...
                    coco_base_path = os.path.join(config["storage-mount-path"], "work/%s/data_platform/%s/%s" % (username,oneJob["projectId"],oneJob["datasetId"]))
                    ...
                    os.system("ln -s %s %s" %(find_dataset_bind_path(oneJob["projectId"],oneJob["datasetId"]),os.path.join(os.path.dirname(coco_base_path),"images")))
                else:
                    coco_base_path = os.path.join(config["storage-mount-path"],"storage/data_platform/%s/%s" % (oneJob["projectId"],oneJob["datasetId"]))
                    coco_file_path = os.path.join(coco_base_path,"annotations/instance.json")
