    ...
    <>
      {
        userStatus ?  <MaterialTable
          ...
          columns={[{title: 'Username', field: 'userName'},
            ...
            {title: 'Currently Allocated Device', field: 'usedGPU',type:'numeric'},
            {title: 'Currently Allocated Preemptible Device', field: 'preemptableGPU',type:'numeric', render: (rowData: any) => <span>{rowData['preemptableGPU'] ? rowData['preemptableGPU'] : '0'}</span>},
            {title: 'Currently Idle Device', field: 'idleGPU',type:'numeric'},
            {title: 'Past Month Booked Device Hour', field: 'booked',type:'numeric'},
            {title: 'Past Month Idle Device Hour', field: 'idle',type:'numeric'},
            {title: 'Past Month Idle Device Hour %', field: '',type:'numeric', render: (rowData: any) => currentCluster === 'Lab-RR1-V100' ? null : <span style={{ color: Math.floor((rowData['idle'] / rowData['booked']) * 100) > 50 ? "red" : "black" }}>{rowData['booked'] == '0' ? '-' : Math.floor(((rowData['idle'] || 0) / (rowData['booked'] || 0)) * 100)}</span>, customSort: (a: any, b: any) => {return Math.floor((a['idle'] / a['booked']) * 100) - Math.floor((b['idle'] / b['booked']) * 100)}}
          ]}
          data={showCurrentUser ? userStatus.filter((uc: any)=>uc['usedGPU'] > 0 && uc['userName'] !== 'Total') : userStatus}
          options={{filtering: false, paging: false, sorting: true}}
          components={{
