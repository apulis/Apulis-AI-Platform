...
do
  ...
  export HostIP=$(ip route get ${discoverserver} | awk '{print $NF; exit}')
  curl "${homeinserver}/Report?hostIP=$HostIP&sysId=$uuid&clusterId=3924e5ca-00c9-4245-a49f-08f817d1d954&role=$systemrole" || echo "!!!Cannot report to cluster portal!!! Check the internet connection"
  echo "systemId:$uuid"
