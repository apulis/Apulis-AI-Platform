...
do
	...
	export ETCDENDPOINTS=$(wget -q -O - "$homeinserver/GetClusterInfo?clusterId=3924e5ca-00c9-4245-a49f-08f817d1d954&key=etcd_endpoints" | sed 's/"//g' | sed 's/\//\\\//g')
	export APISERVER=$(wget -q -O - "$homeinserver/GetClusterInfo?clusterId=3924e5ca-00c9-4245-a49f-08f817d1d954&key=api_server" | sed 's/"//g' | sed 's/\//\\\//g')
	...
	if [ ! -z "$ETCDENDPOINTS" ] && [ ! -z "$APISERVER" ] && [ "$ETCDENDPOINTS" != "$NSTR" ] && [ "$APISERVER" != "$NSTR" ]; then
		if [ ! -f /opt/bin/kubelet ]; then
			...
			kubeid=$(docker create gcr.azk8s.cn/google-containers/hyperkube:v1.15.2)
