        ...
        if oneJob["type"] == "image" and oneJob["targetFormat"]=="coco":
            try:
                ...
                if judge_datasets_is_private(oneJob["projectId"],oneJob["datasetId"]):
                    ...
                    show_coco_file_path = "/home/%s/data_platform/%s/%s/convert_coco.json" % (username,oneJob["projectId"],oneJob["datasetId"])
                    mkdirs(os.path.dirname(coco_file_path))
                    os.system("ln -s %s %s" %(find_dataset_bind_path(oneJob["projectId"],oneJob["datasetId"]),os.path.join(os.path.dirname(show_coco_file_path),"images")))
                else:
